...
(defvar njord-project-hook nil
  "Hook run when a project is enabled. The name of the project's mode and its
state are passed in.")

(def-package! projectile
  :demand t
  :init (add-hook 'njord-init-hook #'projectile-mode)
  :config
  (setq projectile-cache-file (concat njord-cache-dir "projectile.cache")
        projectile-enable-caching (not noninteractive)
        projectile-file-exists-remote-cache-expire nil
        projectile-indexing-method 'alien
        projectile-known-projects-file (concat njord-cache-dir "projectile.projects")
        ...
        (append (list njord-local-dir ".sync")
                projectile-globally-ignored-directories))

  ;; Projectile root-searching functions can cause an infinite loop on TRAMP
  ;; connections, so disable them.
  (defun njord*projectile-locate-dominating-file (orig-fn &rest args)
    "Don't traverse the file system if a remote connection."
    (unless (file-remote-p default-directory)
      (apply orig-fn args)))
  (advice-add #'projectile-locate-dominating-file :around #'njord*projectile-locate-dominating-file)

  (defun njord*projectile-cache-current-file (orig-fun &rest args)
    ...
    (unless (cl-some (lambda (path)
      ...
      (apply orig-fun args)))
  (advice-add #'projectile-cache-current-file :around #'njord*projectile-cache-current-file))
...
`default-directory'. Recognizes (and ...) and/or (or ...) forms."
  (njord--resolve-paths files (njord-project-root)))
...
(defvar-local njord-project nil
  "A list of project mode symbols to enable. Used for .dir-locals.el.")

(defun njord|autoload-project-mode ()
  "Auto-enable projects listed in `njord-project', which is meant to be set from
.dir-locals.el files."
  (dolist (mode njord-project)
    (funcall mode)))
(add-hook 'after-change-major-mode-hook #'njord|autoload-project-mode)
...
A project can be enabled through .dir-locals.el too, if `njord-project' is set to
    ...
    `(progn
       ...
       ,(when (or modes match files when)
          `(associate! ,name
             ...
             :when ,when))
       (add-hook! ,name
         (run-hook-with-args 'njord-project-hook ',name))
       ,(when init-form
