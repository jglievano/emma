...
(defun njord--resolve-paths (paths &optional root)
  (cond ((stringp paths)
         `(file-exists-p
           (expand-file-name
            ,paths ,(if (or (string-prefix-p "./" paths)
                        ...
                        'default-directory
                      (or root `(njord-project-root))))))
        ((listp paths)
         (cl-loop for i in paths
                  collect (njord--resolve-paths i root)))
        (t paths)))

(defun njord--resolve-hooks (hooks)
  (cl-loop with quoted-p = (eq (car-safe hooks) 'quote)
           for hook in (njord-enlist (njord-unquote hooks))
...
(defun njord-unquote (exp)
  "Return EXP unquoted."
  (while (memq (car-safe exp) '(quote function))
    (setq exp (cadr exp)))
  exp)

(defun njord-enlist (exp)
...
(defmacro quiet! (&rest forms)
  ...
  `(if njord-debug-mode
       (progn ,@forms)
     (fset 'njord--old-write-region-fn (symbol-function 'write-region))
     (cl-letf ((standard-output (lambda (&rest _)))
               ...
               ((symbol-function 'write-region)
                (lambda (start end filename &optional append visit lockname mustbenew)
                  ...
                  (njord--old-write-region-fn
                   start end filename append visit lockname mustbenew)))
               (inhibit-message t)
               (save-silently t))
       ,@forms)))

(defvar njord--transient-counter 0)
...
then it detaches itself."
  ...
  (let ((append (eq (car forms) :after))
        (fn (intern (format "njord-transient-hook-%s" (cl-incf njord--transient-counter)))))
    `(when ,hook
       (fset ',fn
             (lambda (&rest _)
               ,@forms
...
`args'."
  ...
  (let ((hook-fn 'add-hook)
    ...
    (let ((hooks (njord--resolve-hooks (pop args)))
          (funcs
           (let ((val (car args)))
  ...
  (unless noninteractive
    (let ((modes (plist-get plist :modes))
      ...
      (cond ((or files modes pred-form)
             ...
             (let ((hook-name (intern (format "njord--init-mode-%s" mode))))
               `(progn
                  (defun ,hook-name ()
                    (when (and (boundp ',mode)
                               ...
                               ,(if files (njord--resolve-paths files) t)
                               ,(or pred-form t))
                      (,mode 1)))
                  ,@(if (and modes (listp modes))
                        (cl-loop for hook in (njord--resolve-hooks modes)
                                 collect `(add-hook ',hook ',hook-name))
                      `((add-hook 'after-change-major-mode-hook ',hook-name))))))
            (match
             `(push (cons ,match ',mode) njord-auto-minor-mode-alist))
...
executed when called with `set!'. FORMS are not evaluated until `set!' calls it."
  ...
  (let ((fn (intern (format "njord--set%s" keyword))))
    `(progn
       (defun ,fn ,arglist
         ,docstring
         ,@forms)
       (cl-pushnew ',(cons keyword fn) njord-settings :test #'eq :key #'car))))
...
(defmacro set! (keyword &rest values)
  ...
  (let ((fn (cdr (assq keyword njord-settings))))
    (if fn
        (apply fn values)
