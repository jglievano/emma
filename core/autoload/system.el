...
;;;###autoload
(defun njord-system-os (&optional os)
...
;;;###autoload
(defun njord-sh (command &rest args)
  "Runs a shell command and prints any output to the NJORD buffer."
  (let ((cmd-list (split-string command " ")))
    (cond ((equal (car cmd-list) "sudo")
           (apply #'njord-sudo (string-join (cdr cmd-list) " ") args))
          ((let ((bin (executable-find "npm")))
             (and (file-exists-p bin)
                  (not (file-writable-p bin))))
           (apply #'njord-sudo (string-join cmd-list " ") args))
          (t
           (princ (shell-command-to-string (apply #'format command args)))))))

(defvar tramp-verbose)
;;;###autoload
(defun njord-sudo (command &rest args)
  "Like `njord-sh', but runs as root (prompts for password)."
  (let ((tramp-verbose 2))
    (with-current-buffer (get-buffer-create "*njord-sudo*")
      (unless (string-prefix-p "/sudo::/" default-directory)
        (cd "/sudo::/"))
      (princ (shell-command-to-string (apply #'format command args))))))

;;;###autoload
(defun njord-fetch (fetcher location dest)
