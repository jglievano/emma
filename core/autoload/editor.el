...
;;;###autoload
(defun njord/sudo-find-file (file)
  "Open a file as root."
  (interactive
...
;;;###autoload
(defun njord/sudo-this-file ()
  "Open the current file as root."
  (interactive)
  (njord/sudo-find-file (file-truename buffer-file-name)))

(defun njord--goto-first-non-blank ()
  (beginning-of-visual-line)
  (skip-chars-forward " \t\r"))

;;;###autoload
(defun njord/backward-to-bol-or-indent ()
  ...
  (let ((boi (save-excursion (back-to-indentation) (point)))
    ...
    (if (= boi point)
      ...
      (unless (= (line-beginning-position) point)
        (njord--goto-first-non-blank)))))

;;;###autoload
(defun njord/forward-to-last-non-comment-or-eol ()
...
;;;###autoload
(defun njord/backward-kill-to-bol-and-indent ()
...
;;;###autoload
(defun njord/backward-delete-whitespace-to-column ()
    ...
    (cond ((njord--surrounded-p)
           (let ((whitespace-match (match-string 1)))
             (cond ((not whitespace-match)
                   ...
                   (t (just-one-space 0)))))
          (t
           (njord/backward-delete-whitespace-to-column)))))

;;;###autoload
(defun njord/newline-and-indent ()
...
;;;###autoload
(defun njord/retab (&optional beg end)
...
;;;###autoload
(defun njord/toggle-sticky (&optional beg end)
...
;;;###autoload
(defun njord/scratch-buffer (&optional arg)
...
If a region is active, copy it into the scratch buffer."
  ...
  (let ((text (and (region-active-p)
        ...
        (old-project (njord-project-root))
        (new-buf (get-buffer-create "*njord:scratch*")))
    (if arg
        (switch-to-buffer new-buf)
      (njord-popup-buffer new-buf))
...
;;;###autoload
(defun njord|enable-delete-trailing-whitespace ()
