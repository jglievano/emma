...
;;;###autoload
(defun njord-run-tests (&optional modules)
...
If neither is available, run all tests in all enabled modules."
  ...
  (condition-case-unless-debug ex
      (let (targets)
        ;; ensure NJORD is initialized
        (let (noninteractive)
          (load (expand-file-name "core/core.el" user-emacs-directory) nil t)
          (njord-initialize-modules nil))
        ;; collect targets
        (cond ((and command-line-args-left
                    (equal (car command-line-args-left) "--"))
               (cl-loop for arg in (cdr argv)
                        if (equal arg "core")
                        do (push (expand-file-name "test/" njord-core-dir) targets)
                        else
                        collect
              ...
              (t
               (let ((noninteractive t)
                 ...
                 (setq modules (njord--module-pairs)
                       targets (list (expand-file-name "test/" njord-core-dir))))))
        ;; resolve targets to a list of test files and load them
        (cl-loop with targets =
                 (append targets
                         (cl-loop for (module . submodule) in modules
                                  if submodule
                                  collect (njord-module-path module submodule "test/")
                                  else
                                  nconc
                                  (cl-loop with module-name = (substring (symbol-name module) 1)
                                           with module-path = (expand-file-name module-name njord-modules-dir)
